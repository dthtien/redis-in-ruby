...
In order to use `redis-cli` against our own server, we must implement the `COMMAND` command, since it sends it directly after starting. We also need to change how we process client input, to parse RESP arrays of bulk strings. We also need to support inline commands. Finally, we also need to update the responses we write back, and serialize responses following RESP.
...
```
_listing 5.x ttl_command.rb & pttl_command.rb_
...
```
_listing 5.x: command_command.rb_
...
```
_listing 5.x: describe method in all other command classes_
